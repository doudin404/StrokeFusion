...
if __name__ == '__main__':
    ...
    encoder_ckpt = "checkpoints/ready/best_pizza.ckpt"##################
    ...
    dm = SketchsDataModule(
        root_dir=data_dir,
        cache_dir=save_dir,
        data_name="quick_draw/pizza",               # 子文件夹名##############################
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints",
        filename=f"best_pizza_diff_{config_idx}",######################
        save_top_k=1,
        monitor="val_loss",     # 或者你训练时记录的其他指标
        mode="min",
        save_last=True,
    )
    checkpoint_callback.CHECKPOINT_NAME_LAST = f"last_pizza_diff_{config_idx}"########################
    ...
    last_ckpt = "checkpoints/last_pizza_diff_0.ckpt"##############################
