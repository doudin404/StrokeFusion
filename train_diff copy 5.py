...
if __name__ == '__main__':
    ...
    encoder_ckpt = "checkpoints/ready/best_apple.ckpt"##################
    ...
    dm = SketchsDataModule(
        root_dir=data_dir,
        cache_dir=save_dir,
        data_name="quick_draw/apple",               # 子文件夹名##############################
        data_type="npz",               # 数据类型：'svg', 'npz', 'json'
        path_points=64,
        gamma=200.0,
        display_scale=0.8,
        #subset=None,
        batch_size=256,#8192,  # 每个 GPU 的 batch size
        num_workers=20,#20,#############################
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints",
        filename=f"best_apple_diff_{config_idx}",######################
        save_top_k=1,
        monitor="val_loss",     # 或者你训练时记录的其他指标
        mode="min",
        save_last=True,
    )
    checkpoint_callback.CHECKPOINT_NAME_LAST = f"last_apple_diff_{config_idx}"########################
    ...
    last_ckpt = "checkpoints/last_apple_diff_0.ckpt"##############################
